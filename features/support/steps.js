...
When("the delete button is pressed", async function () {
  deleteButtons = await this.waitForElements(By.name("delete"))
  await deleteButtons.pop().click()
  this.driver.sleep(4000)
})

Then("a tip with title {string} is not shown", async function (string) {
...
When("{string} is entered in the add description field",
  async function (string) {
    tagFields = await this.waitForElements(By.name("description"))
    this.driver.sleep(1000)
...
Then("a tip with the description {string} is shown", async function (string) {
