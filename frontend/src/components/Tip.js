...
const Tip = props => {
  ...
  return (
    <div className="tip">
      ...
      {tipState !== TipState.EDIT && (
        <Fab name="edit" onClick={() => setTipState(TipState.EDIT)}>
          <EditIcon />
        </Fab>
      )}
      {tipState === TipState.EDIT && (
        <Fab name="cancel" onClick={cancel}>
          <CloseIcon />
        </Fab>
      )}

      {tipState === TipState.NODETAILS && (
        <Fab name="details" onClick={() => setTipState(TipState.DETAILS)}>
          <MoreHorizIcon />
        </Fab>
      )}
      {tipState === TipState.DETAILS && (
        <Fab name="nodetails" onClick={() => setTipState(TipState.NODETAILS)}>
          <CloseIcon />
        </Fab>
      )}
      {tipState === TipState.EDIT && (
        <Fab name="save" onClick={save}>
          <DoneIcon />
        </Fab>
      )}
    </div>
